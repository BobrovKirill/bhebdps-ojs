...
- каталог `src`:
  ...
  - каталог `js`
    - файл `Weapon.js` (в качестве содержимого используйте код из предыдущего задания)
  ...
  - файл `Weapon.js` (Webpack entry point)
...
1. Файл `Game.js` не импортирует ничего
2. Файл `Arm.js` импортирует класс `Game` (используется для наследования)
3. Файл `Bow.js` импортирует класс `Game` (используется для наследования)
4. Файл `Sword.js` импортирует класс `Game` (используется для наследования)
5. Файл `Knife.js` импортирует класс `Game` (используется для наследования)
6. Файл `Staff.js` импортирует класс `Game` (используется для наследования)
...
10. Файл `Weapon.js` импортирует класс `Arm` (используется как начальное оружие игрока) и класс `Knife` (используется для смены оружия)
...
17. Файл `Weapon.js` импортирует все классы персонажей (п.п. 11-16)
18. Файл `Weapon.js` импортирует функцию `play`
